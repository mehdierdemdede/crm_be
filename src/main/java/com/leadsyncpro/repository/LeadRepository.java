...
public interface LeadRepository extends JpaRepository<Lead, UUID>, JpaSpecificationExecutor<Lead> {
  ...
  @Query("""
      ...
      """)
  List<Object[]> countTodayAssignmentsByUsers(@Param("orgId") UUID orgId,
      @Param("dayStart") Instant dayStart);

  long countByAssignedToUser_IdAndCreatedAtBetween(UUID userId, Instant start, Instant end);

  long countByAssignedToUser_IdAndCreatedAtAfter(UUID userId, Instant createdAt);
  ...
  List<Lead> findAllByOrganizationIdAndAdId(UUID organizationId, String adId);

  @Query("SELECT FUNCTION('TO_CHAR', l.createdAt, 'YYYY-MM-DD'), COUNT(l) FROM Lead l WHERE l.organizationId = :orgId AND l.createdAt BETWEEN :start AND :end GROUP BY FUNCTION('TO_CHAR', l.createdAt, 'YYYY-MM-DD')")
  List<Object[]> countByDateBetween(@Param("orgId") UUID orgId, @Param("start") Instant start,
      @Param("end") Instant end);
